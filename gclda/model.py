...
class Model(object):
    ...
    def update_regions(self):
        ...
        if not self.symmetric:
            ...
            for t in range(self.nt):
                ...
                # -- Estimate the Weighed means of all dims, where for dim1 we
    ...
    def compute_log_likelihood(self, dat, update_vectors=True):
        """
        ...
        with open(outfilestr, 'w+') as fid:
            ...
            for i in range(self.ny):
                ...
                outstr = '%d,%d,%d,%d,%d' % (self.dat.peak_vals[i, 0],
                                             self.dat.peak_vals[i, 1],
                                             self.dat.peak_vals[i, 2],
                                             self.yidx[i]+1,
                                             self.ridx[i]+1)
                fid.write(outstr)
                fid.write('\n')
